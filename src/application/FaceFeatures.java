...
public class FaceFeatures {
	public Mat displayFaceFeatures(Mat inImage) {
		...
		for (int i = 0; i < faces.length; i++) {
			...
			Imgproc.rectangle(inImage, faces[i].tl(), faces[i].br(),
			...
			if (faces[i] != null) {
				...
				for (int j = 0; j < eyes.length; j++) {
					...
					Point pt1 = new Point(faces[i].tl().x + eyes[j].tl().x,
							faces[i].tl().y + eyes[j].tl().y);
					// Point pt2 = new Point(pt1.x + eyes[j].width,pt1.y +
					// eyes[j].height);
					Point leye = new Point(pt1.x + 2, pt1.y + eyes[j].height
							/ 2);
					// Point leye2 = new Point(leye.x + eyes[j].width-2,leye.y);
					Point reye = new Point(pt1.x + eyes[j].width - 2, pt1.y
							+ eyes[j].height / 2);
					// Point reye2 = new Point(pt1.x + eyes[j].width -2,pt1.y +
					// eyes[j].height/2);
					...
					// Imgproc.rectangle(inImage, pt1, pt2, new
					// Scalar(255,0,0),1);
					// for(int k = 0; k < eyeCorners.length; k++)
					// {

					// Point temp = new Point(pt1.x+eyeCorners[k].x,pt1.y +
					// eyeCorners[k].y);
			...
			if (nose.length != 0) {
				...
				for (int m = 0; m < nose.length; m++) {
					nosePt1 = new Point(faces[i].tl().x + nose[m].tl().x,
							faces[i].tl().y + nose[m].tl().y);
					Point pt2 = new Point(nosePt1.x + nose[m].width, nosePt1.y
							+ nose[m].height);
					// Imgproc.rectangle(inImage, nosePt1, pt2, new
					// Scalar(255,255,0),1);
				}
				for (int l = 0; l < mouth.length; l++) {
					Point pt1 = new Point(faces[i].tl().x + mouth[l].tl().x,
							faces[i].tl().y + mouth[l].tl().y);
					Point pt2 = new Point(pt1.x + mouth[l].width, pt1.y
							+ mouth[l].height);
					if (pt1.y > nosePt1.y)
		...
		for (int i = 0; i < eyePoints.size(); i++) {
			Imgproc.circle(inImage, eyePoints.get(i), 3, new Scalar(0, 255, 0),
					-1);
		}
